# To:  detect regions where sign of change is inconsistent between variables...  >>> Fig.5a
#       - Global maps
...
data_directory_main = os.path.join(data_directory_top, 'isimip2b.standardized_drought',
                                   'climate_indices_postprocessed_monthly_crop_calendar',
                                   base, version,
...
figure_directory_main = os.path.join(data_directory_top, 'figure_box', 'isimip2b.standardized_drought',
                                     'basicMaps.separate.find_inconsisnteces_variables_crop',
                                     #base, version,
                                     #'climatology_{}'.format(climatology),
                                     'clm{}'.format(climatology),
                                     #today
                                     )
...
# ----------------------------------------------------------------------------------------------------------------------
def read_netcdf(item_compared_about, item_investigate_about, drought_severity, scale, distribution, ghm, gcm, scenario, crop_type):
    ...
    if not DUMMY:  # src.shape (ny, nx, ntime)  ntime = 2868 = 12*239
        ...
        crop_id = dict_crop_id[crop_type]
        if variable == 'pr':
            filename = f'{gcm}_hist{scenario}_pr_monthly_1861_2099_spi_{distribution}_{scale:02}_{drought_severity}_{proxy}_{crop_id:02}.{crop_type}.nc'
        else:
            soc_hist, soc_future = dict_soc[ghm]
            filename = f'{ghm}_{gcm}_hist{scenario}_{soc_hist}_{soc_future}_co2_{variable}_monthly_1861_2099_spi_{distribution}_{scale:02}_{drought_severity}_{proxy}_{crop_id:02}.{crop_type}.nc'
...
def write2nc(src, item1, item2, comparison, item_investigate_about,
             scenario, period, ensemble_type, distribution, scale, drought_severity, crop_type):
    ...
    crop_id = dict_crop_id[crop_type]
    filename = f'inconsisntentSign-{comparison}_{item1}-{item2}__{item_investigate_about}_{scenario}_{period}__{distribution}_{scale}_{drought_severity}_{crop_id:02}_{crop_type}.nc'
    # output_directory = genpath(figure_directory_main, scenario, ensemble_type, drought_severity, distribution, str(scale))
    output_directory = genpath(figure_directory_main, str(scale))
    ...
    times.units              = 'days since {}-01-01 00:00:00'.format(baseyear)
...
def draw_inconsistence_map(src, significance_mask, item1, item2, period,
                           ...
                           scenario, distribution, scale, drought_severity, crop_type):
    ...
    colors1 = divide([[255, 255, 255], [152., 148., 201.], [255., 255., 182.], [241., 163., 135.], [142., 219.,216.]], 255.)  # purple, yellow, orange, bluegreen
    ...
    colors2 = divide([[255, 255, 255], [22., 15., 121.], [230., 230., 0.], [199., 56., 4.], [0., 148., 143.]], 255.)  # purple, yellow, orange, bluegreen
    ...
    # save
    crop_id = dict_crop_id[crop_type]
    figurename = f'inconsisntentSign-{comparison}_{item1}-{item2}__{item_investigate_about}_{scenario}_{period}__{distribution}_{scale}_{drought_severity}_{crop_id:02}_{crop_type}.'
    #figure_directory = genpath(figure_directory_main, scenario, ensemble_type, drought_severity, distribution, str(scale))
    figure_directory = genpath(figure_directory_main, str(scale))
...
def main(*args):
    ...
    for item_investigate_about, scenario, ensemble_type, distribution, scale, drought_severity, crop_type in itertools.product(items_investigate_about, 
                                                                                                                               scenarios, ensemble_types, 
                                                                                                                               distributions, scales, drought_severities,
                                                                                                                               crop_types):
        print('\nJob started !!!')
        print('\n\n===================\n {} {} {} {} {} {} {}\n===================\n'.format(scenario, ensemble_type,
                                                                                             distribution, scale, drought_severity,
                                                                                             item_investigate_about, crop_type))
        strTime = datetime.datetime.now()
        ...
        for item_compared_about in items_compared_about:
            ...
            # Reading data
            srcs = array([[read_netcdf(item_compared_about, item_investigate_about, drought_severity, scale, distribution, ghm, gcm, scenario, crop_type) for ghm in ghms] for gcm in gcms])  # (ngcm, nghm, nperiod, ny, nx)
            ...
            dict_significance_indevidual[item_compared_about] = ma.make_mask(np.abs(dict_ensemble_data[item_compared_about]) < 10)  # make masks, if < 10%
