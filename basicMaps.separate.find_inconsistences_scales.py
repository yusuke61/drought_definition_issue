# To:  detect regions where sign of change is inconsistent between scales...  >>> Fig.2, SupFig.4&5
#       - Global maps
...
if 'scs' in hostname:
    ...
    raise ValueError('data_directory_top...')
...
figure_directory_main = os.path.join(data_directory_top, 'figure_box', 'isimip2b.standardized_drought',
                                     'basicMaps.separate.find_inconsisnteces_scales',
                                     #base, version,
                                     #f'climatology_{climatology}',
                                     f'clm{climatology}',
                                     #today
...
# ----------------------------------------------------------------------------------------------------------------------
def read_netcdf(variable, item_investigate_about, drought_severity, scale, distribution, ghm, gcm, scenario):

    if not DUMMY:  # src.shape (ny, nx, ntime)  ntime = 2868 = 12*239
        if comparison == 'scale':
            proxy = item_investigate_about
        #elif comparison == 'proxy':
        #    variable = item_investigate_about; proxy = item_compared_about
        if variable == 'pr':
...
def write2nc(src, item1, item2, comparison, item_investigate_about,
             scenario, period, ensemble_type, distribution, variable, drought_severity):
    ...
    filename = f'basicMap.ensemble{ensemble_type}_inconsisntentSign-{variable}_{comparison}_{item1}-{item2}__{item_investigate_about}_{scenario}_{period}__{distribution}_{drought_severity}.nc'
    output_directory = genpath(figure_directory_main, scenario, ensemble_type, drought_severity, distribution, variable)
    ...
    srcs.memo                = 'scales      : ' + ', '.join([str(item1), str(item2)])
    srcs.memo1               = 'drought type: ' + ', '.join(variable)
...
def draw_inconsistence_map(src, significance_mask, item1, item2, period,
                           ...
                           scenario, distribution, variable, drought_severity):

    fig1 = plt.figure(num=1, figsize=(4, 1.7))
    gs = gridspec.GridSpec(1, 1)  # (rows,cols)
    gs.update(left=0.01, right=0.99, bottom=0.03, top=0.98, hspace=0.02, wspace=0.01)
    ...
    colors1 = divide([[188, 154, 144], [152., 148., 201.], [142., 219., 216.], [241., 163., 135.], [255., 255., 182.]], 255.)  # purple, bluegreen, orange, yellow
    ...
    colors2 = divide([[188, 154, 144], [22., 15., 121.], [0., 148., 143.], [199., 56., 4.], [230., 230., 0.]], 255.)  # purple, bluegreen, orange, yellow
    ...
    # label
    fontsize = 9.5
    stdname1, stdname2 = f'{dict_standerdizedname[variable]}{item1}', f'{dict_standerdizedname[variable]}{item2}'
    ...
    figurename = f'basicMap.ensemble{ensemble_type}_inconsisntentSign-{variable}_{comparison}_{item1}-{item2}__{item_investigate_about}_{scenario}_{period}__{distribution}_{drought_severity}.'
    figure_directory = genpath(figure_directory_main, scenario, ensemble_type, drought_severity, distribution, variable)
...
def main(*args):
    ...
    print('START basicMap.separate.find_inconsistences_scales.py\n@{}\n'.format(strTIME.strftime("%Y-%m-%d %H:%M:%S")))

    if comparison == 'scale':
        items_compared_about, items_investigate_about = scales, proxies
    elif comparison == 'proxy':
        items_compared_about, items_investigate_about = proxies, scales
    combinations = list(itertools.combinations(scales, 2))

    for item_investigate_about, scenario, ensemble_type, distribution, variable, drought_severity in itertools.product(items_investigate_about, 
                                                                                                                       scenarios, ensemble_types, 
                                                                                                                       distributions, variables, drought_severities):
        ...
        for scale in scales:
            ...
            # Reading data
            srcs = array([[read_netcdf(variable, item_investigate_about, drought_severity, scale, distribution, ghm, gcm, scenario) for ghm in ghms] for gcm in gcms])  # (ngcm, nghm, nperiod, ny, nx)
            ...
            dict_ensemble_data[scale] = ma.masked_equal(mmed_climatological_differences_percent, 1e+20)  # (nperiod,ny,nx)
            dict_significance_indevidual[scale] = ma.make_mask(np.abs(dict_ensemble_data[scale]) < 10)  # masked, if |change| < 10%

        dict_consistences = {}
        dict_significance_combined = {}
        for (item1, item2) in combinations:

            if item1 == 1 and item2 == 3:  # for the paper figure (Fig2)
                item1, item2 = 3, 1
        ...
        for (item1, item2), (i_period, period) in itertools.product(combinations, enumerate(periods[1:])):
            ...
            draw_inconsistence_map(src, significance_mask, item1, item2, period,
                                   comparison, ensemble_type, item_investigate_about, 
                                   scenario, distribution, variable, drought_severity)
